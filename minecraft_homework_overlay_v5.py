...
class TaskFactory:
    @staticmethod
    def build_bilingual(title_ru, title_pl, prompt_ru, prompt_pl, hint_ru, hint_pl, task_type, answer=None, options=None, record_audio=False, voice_prompt=None, allow_hint=True):
        return {
            "type": task_type,
            "title_ru": title_ru,
            "title_pl": title_pl,
    ...
    def make_reading_task(grade, focus_lang):
        ...
        if focus_lang == "ru":
            ...
            return TaskFactory.build_bilingual(
                ...
                voice_prompt="Прочитай, что написано, и нажми на кнопку.", allow_hint=False,
            )
...
class HintGateDialog(tk.Toplevel):
    def __init__(self, parent, grade, on_success, on_close=None):
...
class HomeworkOverlay:
    ...
    def show_overlay(self):
        ...
        try:
            ...
            if not (self.hint_dialog and self.hint_dialog.winfo_exists()):
    ...
    def keep_overlay_active(self):
        ...
        if self.hint_dialog and self.hint_dialog.winfo_exists():
            try:
                self.hint_dialog.attributes("-topmost", True)
                self.hint_dialog.lift()
                self.hint_dialog.focus_force()
            except Exception:
                pass
        elif self.current_task and self.current_task.get("type") == "input":
            self.focus_entry()
    ...
    def open_hint_gate(self):
        ...
        if self.hint_dialog and self.hint_dialog.winfo_exists():
            try:
                ...
                pass
            return
        self.hint_dialog = HintGateDialog(self.root, self.selected_grade, self.reveal_hint, self.on_hint_dialog_closed)
    ...
    def load_next_task(self):
        ...
        if self.current_task.get("allow_hint", True):
            self.hint_btn.pack()
        ...
        if task_type == "input":
            ...
            if self.current_task.get("voice_prompt"):
                speak_async_windows(self.current_task.get("voice_prompt"))
            self.status_label.config(text="Прочитай и нажми кнопку / Przeczytaj i kliknij przycisk", fg=TEXT)

            ...
            if isinstance(data, list):
                ...
                for item in data:
                    ...
                    cleaned.append({
                        ...
                        "type": str(item.get("type", "input") or "input"),
                        ...
                        "options": [str(x) for x in item.get("options", [])] if isinstance(item.get("options", []), list) else [],
...
class TaskFactory:
    ...
    def random_task(grade, focus_lang, custom_tasks=None):
        ...
        if matching_custom and random.random() < 0.5:
...
class TaskEditorDialog(tk.Toplevel):
    def __init__(self, parent, tasks, on_save, current_task=None, current_grade=1):
        super().__init__(parent)
        self.parent = parent
        ...
        tk.Label(left, text="Список заданий", font=("Arial", 16, "bold"), bg=CARD_BG, fg=TEXT).pack(padx=12, pady=(12, 8), anchor="w")
        self.listbox = tk.Listbox(left, width=34, font=("Arial", 12))
        ...
        grid = tk.Frame(form, bg=CARD_BG)
        grid.pack(fill="x")
        for i in range(4):
            grid.grid_columnconfigure(i, weight=1)

        def mk_label(row, col, text_):
            tk.Label(grid, text=text_, font=("Arial", 11, "bold"), bg=CARD_BG, fg=TEXT).grid(row=row, column=col, sticky="w", padx=6, pady=(6, 2))

        mk_label(0,0,"Класс")
        self.grade_var = tk.StringVar(value=str(current_grade))
        tk.OptionMenu(grid, self.grade_var, "1", "2", "3", "4").grid(row=1, column=0, sticky="ew", padx=6)

        mk_label(0,1,"Тип")
        self.type_var = tk.StringVar(value="input")
        tk.OptionMenu(grid, self.type_var, "input", "choice", "button").grid(row=1, column=1, sticky="ew", padx=6)

        mk_label(0,2,"Ответ")
        self.answer_entry = tk.Entry(grid, font=("Arial", 12))
        self.answer_entry.grid(row=1, column=2, sticky="ew", padx=6)

        mk_label(0,3,"Варианты (через запятую)")
        self.options_entry = tk.Entry(grid, font=("Arial", 12))
        ...
        tk.Label(fields, text="Tekst справа (polski)", font=("Arial", 11, "bold"), bg=CARD_BG, fg=TEXT).grid(row=2, column=1, sticky="w", padx=6)
        ...
        tk.Button(action_bar, text="Закрыть", command=self.destroy, bg="#9AA8B5", fg="white", relief="flat", font=("Arial", 12, "bold"), padx=12, pady=8).pack(side="right")
    ...
    def refresh_list(self):
        ...
        for idx, task in enumerate(self.tasks, start=1):
            label = f"{idx}. [{task.get('grade',1)} кл] {task.get('title_ru','Без названия')}"
    ...
    def task_from_form(self):
        ...
        return {
            ...
            "voice_text": self.voice_entry.get().strip() or "Сделай задание и нажми кнопку.",
        }
...
class HomeworkOverlay:
    ...
    def create_tray_image(self):
        if not TRAY_AVAILABLE:
            return None
        img = Image.new("RGBA", (64, 64), (106, 140, 175, 255))
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle((6, 6, 58, 58), radius=12, fill=(106, 140, 175, 255))
        draw.rectangle((18, 18, 46, 46), fill=(232, 195, 93, 255))
        draw.rectangle((24, 24, 40, 40), fill=(247, 250, 252, 255))
        return img
    ...
    def hide_to_tray_or_taskbar(self):
        ...
        if TRAY_AVAILABLE:
            try:
                if self.tray_icon:
                    return
                menu = pystray.Menu(
                    pystray.MenuItem("Открыть / Otwórz", self.restore_from_tray),
                    pystray.MenuItem("Выход / Wyjdź", self.exit_from_tray),
                )
                self.tray_icon = pystray.Icon("minecraft_coach_kids", self.create_tray_image(), APP_TITLE, menu)
                self.tray_thread = threading.Thread(target=self.tray_icon.run, daemon=True)
                self.tray_thread.start()
                log_message("Hidden to system tray")
                return
            except Exception as e:
                log_message(f"Tray fallback to taskbar: {e}")
                self.tray_icon = None
        # fallback: обычное сворачивание на панель задач
        self.root.deiconify()
        self.root.iconify()
        log_message("Hidden to taskbar")

    def select_grade(self, grade):
        self.selected_grade = grade
        self.next_break_ts = self.make_next_break_time()
        self.status_label.config(text=f"Выбран уровень: {GRADE_CONFIGS[grade]['label']}. Программа свёрнута до следующей паузы.", fg=TEXT)
        self.hide_task_card()
        self.root.after(250, self.hide_to_tray_or_taskbar)
    ...
    def open_shop(self):
        ...
        self.store_window.protocol("WM_DELETE_WINDOW", lambda: (self.popup_closed("shop"), self.store_window.destroy()))
    ...
    def open_stats(self):
        ...
        self.stats_window.protocol("WM_DELETE_WINDOW", lambda: (self.popup_closed("stats"), self.stats_window.destroy()))

    def save_custom_tasks_from_editor(self, tasks):
        self.custom_tasks = tasks
        save_custom_tasks(tasks)
        self.status_label.config(text=f"Пользовательские задания сохранены: {len(tasks)}", fg=ACCENT_2)
    ...
    def open_task_editor(self):
        ...
        self.task_editor_window = TaskEditorDialog(self.root, self.custom_tasks, self.save_custom_tasks_from_editor, current_task=current, current_grade=grade)
        self.task_editor_window.protocol("WM_DELETE_WINDOW", lambda: (self.popup_closed("editor"), self.task_editor_window.destroy()))

    ...
    def start(self):
        ...
        class Handler(_v16_http.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if password != str(app.settings.get('parent_password', '1234')):
                    return self._html('<html><body style="font-family:Arial;padding:24px;background:#eef3f7"><h2>Панель родителя</h2><form method="GET" action="/"><input type="password" name="password" placeholder="Пароль" style="font-size:18px;padding:8px"><button style="font-size:18px;padding:8px 14px">Войти</button></form></body></html>')
                ...
                body = f"""<html><head><meta charset='utf-8'><title>Parent Panel</title></head><body style='font-family:Arial;background:#eef3f7;color:#233548;margin:0'><div style='max-width:1100px;margin:0 auto;padding:20px'><h2>Панель родителя / Parent panel</h2><p><b>Монеты:</b> {app.stats.get('coins',0)} | <b>Решено:</b> {app.stats.get('total_tasks_completed',0)} | <b>Ошибок:</b> {app.stats.get('wrong_answers',0)}</p><p><b>Адрес для телефона:</b> {_v16_html.escape(app.parent_panel_url or '')}</p><h3>Последние события</h3><ul>{hist}</ul><div style='display:grid;grid-template-columns:1fr 1fr;gap:18px'><form method='POST' action='/save_tasks'><input type='hidden' name='password' value='{_v16_html.escape(password)}'><h3>Задания JSON</h3><textarea name='payload' style='width:100%;height:360px;font-family:Consolas'>{tasks_json}</textarea><br><button style='margin-top:8px;padding:10px 14px'>Сохранить задания</button></form><form method='POST' action='/save_supports'><input type='hidden' name='password' value='{_v16_html.escape(password)}'><h3>Подсказки и правила JSON</h3><textarea name='payload' style='width:100%;height:360px;font-family:Consolas'>{supports_json}</textarea><br><button style='margin-top:8px;padding:10px 14px'>Сохранить подсказки</button></form></div><p style='margin-top:18px'><a href='/api/stats?password={_v16_urlparse.quote(password)}'>Открыть stats JSON</a></p></div></body></html>"""
                self._html(body)
            def do_POST(self):
...
class _V17SoundManager:
    ...
    def start_music(self):
        ...
        try:
            ...
            log_message(f'v17 music start error: {e}')
    ...
    def stop_music(self):
        ...
        try:
            ...
            log_message(f'v17 music stop error: {e}')
    ...
    def play_correct(self):
        ...
        try:
            ...
            log_message(f'v17 correct sound error: {e}')
    ...
    def play_wrong(self):
        ...
        try:
            ...
            log_message(f'v17 wrong sound error: {e}')

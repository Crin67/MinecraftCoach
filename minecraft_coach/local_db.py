...
from .local_db_support import (
    BUILT_IN_CONTENT_STATE_KEY,
    DEFAULT_PARENT_PASSWORD,
    DEFAULT_SETTINGS,
    SCHEMA_VERSION,
    accepted_answers_from_task as accepted_answers_for_task,
    answer_display as answer_to_text,
    json_from_text as _json_loads,
    json_to_text as _json_dumps,
    normalize_stats_payload,
    normalize_task_records,
    public_settings_dict,
    relative_to_or_none as _path_relative_to,
...
class LocalDB:
    ...
    def _topic_defaults_for_task(self, task: dict[str, Any]) -> tuple[str, str | None]:
        return task_topic_defaults(task)
    ...
    def _normalize_tasks(self, tasks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return normalize_task_records(tasks)

    def _normalize_stats(self, stats: dict[str, Any]) -> dict[str, Any]:
        return normalize_stats_payload(stats)
    ...
    def _accepted_answers_from_task(self, task: dict[str, Any]) -> list[str]:
        return accepted_answers_for_task(task)
    ...
    def _public_settings(self, settings: dict[str, Any] | None = None) -> dict[str, Any]:
        return public_settings_dict(settings or self.get_settings())

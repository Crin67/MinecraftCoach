...
class HomeworkOverlay:
    ...
    def show_result_popup(self, success, text):
        ...
        self.toast_window.attributes("-fullscreen", True)
        self.toast_window.configure(bg="#DDE7EE")

        overlay = tk.Frame(self.toast_window, bg="#DDE7EE")
        overlay.pack(fill="both", expand=True)

        center_shadow = tk.Frame(overlay, bg=SHADOW)
        center_shadow.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.52, relheight=0.34)

        inner_bg = "#EAF7EE" if success else "#FCECEF"
        edge = ACCENT_2 if success else BAD
        icon = "✓" if success else "✕"

        card = tk.Frame(center_shadow, bg=inner_bg, highlightthickness=3, highlightbackground=edge)
        card.pack(fill="both", expand=True, padx=(0, 8), pady=(0, 8))

        tk.Label(card, text=icon, font=("Arial", 76, "bold"), bg=inner_bg, fg=edge).pack(pady=(34, 8))
        tk.Label(card, text=text, font=("Arial", 28, "bold"), bg=inner_bg, fg=TEXT, justify="center").pack(padx=40, pady=(0, 26))
        tk.Label(card, text="Подготовься к следующему заданию", font=("Arial", 16), bg=inner_bg, fg=SUBTEXT).pack(pady=(0, 24))

        try:
            self.toast_window.lift()
            self.toast_window.focus_force()
        except Exception:
            pass
